...
//		{
//			if(!(((para->dram_tpr13 & (0x1u<<31)) == (0x1<<31)) && (((para->dram_para2>>8)&0xF) == 0x1)))
//				mctl_dll_init(1, para);
//		}
	}else
	{
...
/*
	if((para->dram_tpr13 & (0x1u<<31)) == (0x0<<31))
	{
		...
		if((((para->dram_tpr13>>3)&0x3) == 0) && (size_lock == 1))//A31
		{
			if(dram_size != 2048)
			{
...
/*
	...
	if((para->dram_tpr13 & (0x1<<1)) == (1<<1))//channel lock
		ch_lock = 1;
*/
	if((para->dram_tpr13 & (0x1u<<31)) == 0)//auto detect
	{
...
unsigned int mctl_com_init(__dram_para_t *para)
{
	...
	if(((para->dram_para2>>8)&0xF) == 1)//1 ch
	{
		reg_val = mctl_read_w(SDR_COM_CCR);	//turn off ch1 all DLL
		reg_val |= (0x1<<4);
		reg_val &= ~(0x1<<1);
...
signed int init_DRAM(int type, void *para)
{
	...
	msg("[DRAM]ver 1.01 clk = %d\n", dram_para->dram_clk  );
//	msg("[DRAM]ver 1.01 para1 = %x\n", dram_para->dram_para1  );
//	msg("[DRAM]ver 1.01 para2 = %x\n", dram_para->dram_para2  );

/*
	//bonding ID:  0: A31	1: A31S		2: A3X PHONE
	id = ss_bonding_id();
	...
	if(id == 0)
	{
		...
		dram_para->dram_tpr13 &= ~(0x3<<3); //id = 00b
	}else if(id == 1)
	{
		dram_para->dram_tpr13 |= 0x2<<7; //1GB max
		dram_para->dram_tpr13 |= (0x1<<3); //id = 01b
		paraconfig(&(dram_para->dram_para2), 0xF<<8, 1<<8);	//ch_num = 1
	}else if(id == 2)
	{
		dram_para->dram_tpr13 |= (0x2<<3); //id = 10b
	}else
	{
		dram_para->dram_tpr13 |= (0x3<<3); //id = 11b
	}
*/
